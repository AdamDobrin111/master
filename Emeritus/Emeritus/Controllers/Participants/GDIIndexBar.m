...
#define kDefaultFontName @"Avenir"
#define kDefaultFontSize 2.f
#define kDefaultTextSpacing 15.f
#define kDefaultTruncatedRowText @"•"
#define kStandardButtonWidth 8.f
...
+ (void)initialize
{
    if (self == [GDIIndexBar class]) {
        ...
        if ([self isOS7OrLater]) {
            UIOffset offset = UIOffsetMake(00.0f, 0.f);
...
- (CGRect)rectForIndexBarFrame
{
    ...
    CGPoint origin = CGPointMake(relativeTableViewTopRightPoint.x - _barWidth,
                                 relativeTableViewTopRightPoint.y + _tableView.contentOffset.y + _tableView.contentInset.top+5);
    
    CGFloat height = _tableView.frame.size.height - (_tableView.contentInset.top-5 + _tableView.contentInset.bottom);
...
{
    //    if ([self.delegate respondsToSelector:@selector(indexBar:didSelectIndexString:)]/*[self.delegate respondsToSelector:@selector(indexBar:didSelectIndex:)]*/) {
    ...
    NSString *name = _indexStrings[stringIndex];
    if (stringIndex != _lastSelectedStringIndex) {
        [self.delegate indexBar:self didSelectIndexString:stringIndex];
