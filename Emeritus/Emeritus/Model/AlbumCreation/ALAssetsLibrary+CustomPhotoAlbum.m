...
- (void)addAssetURL:(NSURL *)assetURL
         ...
         completion:(ALAssetsLibraryWriteImageCompletionBlock)completion
            failure:(ALAssetsLibraryAccessFailureBlock)failure
{
    __block BOOL albumWasFound = NO;   // Signature for the block executed when a match is found during enumeration using
...
-(void)getImageFromAssetUrl:(NSURL *)assetUrl withCompletionBlock:(GetImageFromAssetUrlCompletionBlock)completionblock AndFailureBlock:(FailureBlockForGetImageFromAssetUrl)failureBlock
{
    [self assetForURL:assetUrl resultBlock:^(ALAsset *asset) {
        ...
        if (iref)
        {
            UIImage* imageFromAsset=[UIImage imageWithCGImage:iref scale:1.0 orientation:UIImageOrientationUp];
            //           UIImage *rotatedImage= [UIImage rotateUIImage:imageFromAsset];
            completionblock(imageFromAsset);
        }
...
{
    [self saveImage:image toAlbum:albumName completion:^(NSURL *assetURL, NSError *error) {
...
{
    [self saveVideo:videoUrl toAlbum:albumName completion:^(NSURL *assetURL, NSError *error) {
...
-(void)checkMediaExistedOrNot:(NSURL *)mediaUrl WithStatusBlock:(MediaExisted)existedBlock
{
    ...
    [self assetForURL:mediaUrl resultBlock:^(ALAsset *asset) {
        // If asset exists
        if (asset) {
